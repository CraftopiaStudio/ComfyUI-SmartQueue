...
- **Quick interruptions**: you queued 50 renders and halfway through realize you need to test a different checkpoint or model settings. One click pauses the queue (everything still pending stays in the same order), you queue your test render, run it, then resume the original batch right where it left off.
...
- **Show in Explorer**: right-click a history thumbnail to jump straight to the rendered file on disk, selected and ready, in your OS file manager: no more digging through ComfyUI's output folder by hand.
